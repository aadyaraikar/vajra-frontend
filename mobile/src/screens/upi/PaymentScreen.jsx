...
export default function PaymentScreen({ navigation, route }) {
  ...
  const canProceed = useMemo(() => Number(amount || 0) > 0, [amount]);
      ...
      <View style={styles.screen}>
        ...
        <View style={styles.recipientArea}>
          <View style={styles.avatar}>
            <Text style={styles.avatarText}>{initialFromName(recipient.name)}</Text>
          </View>
          <Text style={styles.recipientName}>{recipient.name}</Text>
          <Text style={styles.recipientMeta}>{recipient.vpa} • {recipient.bank}</Text>
        </View>
